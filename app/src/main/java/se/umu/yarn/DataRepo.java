...
class DataRepo
{
    ...
    private DaoClass userData;
    private LiveData<List<UserModel>> allInterests;
    ...
    DataRepo(Application application) {
        ...
        userData = db.getDao();
        allInterests = userData.getAlphabetUsers();
    }

    // Room executes all queries on a separate thread.
    // Observed LiveData will notify the observer when the data has changed.
    LiveData<List<UserModel>> getAllWords() {
        return allInterests;
    }
    ...
    void insert(UserModel userModel) {
        DatabaseClass.databaseWriteExecutor.execute(() -> {
            userData.insertAllData(userModel);
        });
    }
}